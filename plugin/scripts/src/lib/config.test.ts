...
let testDir: string;
let mockPaths: { SETTINGS_FILE: string; SETTINGS_BACKUP: string; SETTINGS_TEMP: string };
...
const SETTINGS_PATH = (constants as unknown as { PATHS: typeof mockPaths }).PATHS.SETTINGS_FILE;
const BACKUP_PATH = (constants as unknown as { PATHS: typeof mockPaths }).PATHS.SETTINGS_BACKUP;
const TEMP_PATH = (constants as unknown as { PATHS: typeof mockPaths }).PATHS.SETTINGS_TEMP;
const MOCK_DIR = (constants as unknown as { __testDir: string }).__testDir;

describe('config', () => {
  beforeEach(() => {
    // Clean up any existing files
    try { rmSync(SETTINGS_PATH); } catch { /* ignore */ }
    try { rmSync(BACKUP_PATH); } catch { /* ignore */ }
    try { rmSync(TEMP_PATH); } catch { /* ignore */ }
  });
